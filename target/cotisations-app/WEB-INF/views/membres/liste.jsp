...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 style="font-size:20px;font-weight:700;color:var(--primary)">Liste des membres</h2>
        <p class="text-muted">${membres.size()} membre(s) trouvé(s)</p>
    </div>
    <div class="d-flex gap-2">
        <% if (isAdmin) { %>
        <a href="<%= ctx %>/membres?action=excel" class="btn btn-outline btn-sm"><i class="bi bi-file-earmark-excel"></i> Exporter Excel</a>
        <a href="<%= ctx %>/membres?action=nouveau" class="btn btn-primary btn-sm"><i class="bi bi-person-plus-fill"></i> Nouveau membre</a>
        <% } %>
    </div>
</div>
            ...
            <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex;align-items:center;gap:10px">
                                <div style="width:34px;height:34px;background:var(--primary);border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:13px;font-weight:700;flex-shrink:0">
                                    ${m.prenom.charAt(0)}
                                </div>
                                <div>
                                    <div style="font-weight:600">${m.nomComplet}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td>
                            <div style="display:flex;gap:6px">
                                <a href="<%= ctx %>/membres?action=detail&id=${m.id}" class="btn btn-outline btn-sm" title="Voir"><i class="bi bi-eye-fill"></i></a>
                                <% if (isAdmin) { %>
                                ...
                                <c:if test="${m.statut.name() == 'ACTIF'}">
                                    <form method="post" action="<%= ctx %>/membres" style="display:inline"
                                        ...
                                        <button type="submit" class="btn btn-danger btn-sm" title="Désactiver"><i class="bi bi-person-x-fill"></i></button>
                                    </form>
                                </c:if>
                                <% } %>
                            </div>
                ...
                <c:if test="${empty membres}">
                    <tr><td colspan="7" class="text-center" style="padding:48px">
                        <i class="bi bi-people" style="font-size:40px;color:#ccc;display:block;margin-bottom:8px"></i>
                        <span class="text-muted">Aucun membre trouvé</span>
                    </td></tr>
                </c:if>
                </tbody>
            </table>
