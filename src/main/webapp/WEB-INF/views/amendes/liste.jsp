...
<%
    ...
    request.setAttribute("pageTitle", isAdmin ? "Gestion des Amendes" : "Mes Amendes");
    request.setAttribute("activePage", "amendes");
    String ctx = request.getContextPath();
    String devise = (String) request.getAttribute("devise");
    if (devise == null) devise = "FCFA";
    sn.ucad.cotisations.model.Membre sessionMembreA = (sn.ucad.cotisations.model.Membre) session.getAttribute("sessionMembre");
    Long sessionMembreIdA = (sessionMembreA != null) ? sessionMembreA.getId() : null;
%>
<jsp:include page="/WEB-INF/views/common/header.jsp"/>

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 style="font-size:20px;font-weight:700;color:var(--text-main)"><%= isAdmin ? "Gestion des Amendes" : "Mes Amendes" %></h2>
        <p class="text-muted">${amendes.size()} amende(s) enregistrée(s)</p>
    </div>
</div>
                    ...
                    <tr>
                        ...
                        <td>
                            <c:if test="${a.statut.name() != 'PAYEE'}">
                                <c:if test="${sessionScope.sessionRole != 'ADMIN' || (sessionScope.sessionMembre != null && sessionScope.sessionMembre.id == a.membre.id)}">
